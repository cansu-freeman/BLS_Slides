...
df = fred.get_series('VAICLAIMS', observation_start = '2020-03-07', observation_end = '2020-07-14')
df = pd.DataFrame(data = df).reset_index()      #turns series data into dataframe
df.columns = ['date', 'VAICLAIMS']
df['VAICLAIMS_thousands'] = df['VAICLAIMS']/1000
#print(df)

### creating graph 
fig, ax = plt.subplots(figsize = (11, 5.5))
ax.bar(df['date'], df['VAICLAIMS_thousands'], color = 'olivedrab', width = 3, zorder = 3) # for some reason, zorder = 3 helps bars show up in front of grid
...
# labeling points
for x,y in zip(df['date'], df['VAICLAIMS_thousands']):
    label = "{0:.2f}".format(y) # 2 decimal points
    ax.annotate(label, (x,y), textcoords="offset points", xytext=(0,10), ha='center')

# this formats the graph
plt.ylabel('THOUSANDS', fontdict= {'stretch':'condensed', 'variant': 'small-caps'})
plt.ylim(0, 160)
